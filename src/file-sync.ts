...
export async function syncFiles(sourceDir: string, options: SyncOptions): Promise<void> {
  try {
    const verbosity = options.quiet ? Verbosity.Quiet : options.verbose ? Verbosity.Verbose : Verbosity.Normal;
    ...
    const uploader = createUploader(
      ...
      options.target || "/",
      verbosity,
      { internxtService, hashCache, progressTracker, compressionService, resumableUploader }
    );
