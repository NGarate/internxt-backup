...
export async function restoreFiles(options: RestoreOptions): Promise<void> {
  ...
  if (manifest) {
    logger.info(`Loaded backup manifest from ${manifest.timestamp}`, verbosity);
  } else {
    logger.warning(
  ...
  if (options.path) {
    ...
    filesToDownload = filesToDownload.filter((f) => {
      ...
      const relativePath = f.remotePath
        ...
        .replace(/^\//, '');
      return (
        relativePath.startsWith(`${filterPath}/`) || relativePath === filterPath
      );
    });
    logger.info(
  ...
  if (stats.verifiedCount > 0) {
    logger.info(`Checksums verified: ${stats.verifiedCount} files`, verbosity);
  }
