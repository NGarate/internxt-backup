...
describe('File System Utilities', () => {
  ...
  describe('urlEncodePath', () => {
    it('should properly encode path components', () => {
      const paths = {
        ...
        'path with spaces/and special chars':
          'path%20with%20spaces/and%20special%20chars',
    ...
    it('should properly handle empty path components', () => {
      expect(fsUtils.urlEncodePath('path//with/empty/parts')).toBe(
    ...
    it('should normalize backslashes to forward slashes before encoding', () => {
      expect(fsUtils.urlEncodePath('path\\with\\backslashes')).toBe(
        'path/with/backslashes',
      );
      expect(fsUtils.urlEncodePath('mixed/path\\style')).toBe(
        'mixed/path/style',
      );
      expect(fsUtils.urlEncodePath('core\\upload\\file.js')).toBe(
        'core/upload/file.js',
      );
      expect(
        fsUtils.urlEncodePath('path with spaces\\and special\\chars'),
      ).toBe('path%20with%20spaces/and%20special/chars');
    });

    it('should handle windows-style paths correctly', () => {
      expect(fsUtils.urlEncodePath('C:\\Users\\name\\Documents')).toBe(
        'C%3A/Users/name/Documents',
      );
      expect(fsUtils.urlEncodePath('D:\\Projects\\test file.txt')).toBe(
        'D%3A/Projects/test%20file.txt',
      );
    });
  });
  ...
  describe('calculateChecksum', () => {
    it('should calculate MD5 checksum for file', async () => {
      ...
      const mockChecksum = spyOn(
        ...
        Promise.resolve('7f107e29cce7268c1a8d4ef949060e56'),
      );
  ...
  describe('JSON file operations', () => {
    ...
    describe('saveJsonToFile', () => {
      ...
      it('should return false and log error on write failure', async () => {
        spyOn(fsUtils, 'writeFileAsync').mockImplementation(() =>
    ...
    describe('loadJsonFromFile', () => {
      it('should load and parse JSON data from a file', async () => {
        spyOn(fsUtils, 'existsAsync').mockImplementation(() =>
          Promise.resolve(),
        );
        spyOn(fsUtils, 'readFileAsync').mockImplementation(() =>
          Promise.resolve('{"key":"value","number":123}'),
        );

        const result = await fsUtils.loadJsonFromFile<Record<string, unknown>>(
          '/test/data.json',
          {},
        );
      ...
      it('should return default value if file does not exist', async () => {
        ...
        const result = await fsUtils.loadJsonFromFile(
          '/test/data.json',
          defaultValue,
        );
